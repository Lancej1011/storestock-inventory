...
interface ProductBundle {
  id: string;
  quantity: number;
  child: Product & { inventory?: Inventory[] };
}
...
export default function Scan() {
  ...
  const handleLensProductFound = (p: ProductMatch) => {
    const typed = p as unknown as Product;
    setProduct(typed);
    if (typed.bundleParent?.length) setBundle(typed.bundleParent[0]);
    const storeInventory = (typed as Product & { inventory?: Inventory[] })
  ...
  const lookupProduct = async (barcode: string) => {
    ...
    setAutoPopulated(null);
    try {
      // QR labels generated by this app encode SSID:{productId} — look up by ID directly
      const isQRCode = barcode.startsWith('SSID:');
      const endpoint = isQRCode
        ? `/products/${encodeURIComponent(barcode.slice(5))}`
      ...
      if (data.isExternalResult) {
        ...
        setProduct(typed);
        if (typed.bundleParent?.length) setBundle(typed.bundleParent[0]);
        if (typed.autoPopulated && typed.dataSource) {
          setAutoPopulated({ source: typed.dataSource });
        }
  ...
  // ── Bundle / case expansion ────────────────────────────────────────────────

  const handleExpandToUnits = (b: ProductBundle, cases: number) => {
    setProduct(b.child);
    setQuantity(b.quantity * cases);
    setBundle(null);
    const storeInventory = b.child.inventory?.find((inv) => inv.store.id === selectedStoreId);
    setInventory(storeInventory ?? null);
  };
  ...
  const handleCreateProduct = async (forceCreate = false) => {
    ...
    // Duplicate check — skip if user already confirmed they want to create anyway
    if (!forceCreate) {
      try {
        const simRes = await api.get(`/products/similar?name=${encodeURIComponent(newProduct.name)}&threshold=0.5`);
        if (simRes.data.length > 0) {
          setDuplicateCandidates(simRes.data);
          return; // pause and show the warning — user must confirm
        }
      } catch {
        // non-fatal — proceed with creation if the check fails
      }
    }
  ...
  const handleUseExistingProduct = async (candidateId: string) => {
    setLoading(true);
    setError(null);
    try {
      const response = await api.get(`/products/${candidateId}`);
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-24">
      ...
      <div className="max-w-xl mx-auto space-y-6">
        ...
        {!loading && product && bundle && !stockInSuccess && (
          <div className="bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/25 p-5 rounded-2xl animate-fade-in">
            <div className="flex items-start gap-3 mb-4">
              <div className="w-8 h-8 rounded-xl bg-sky-100 dark:bg-sky-900/40 flex items-center justify-center shrink-0">
                <svg className="w-4 h-4 text-sky-600 dark:text-sky-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" />
                </svg>
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-sky-600 dark:text-sky-400 mb-0.5">
                  Case Detected · {bundle.quantity} units per case
                </p>
                <p className="text-sm font-semibold text-sky-800 dark:text-sky-200">
                  <span className="font-black">{product.name}</span> contains{' '}
                  <span className="font-black">{bundle.quantity}× {bundle.child.name}</span>.
            ...
            <div className="flex gap-3">
              <button
                onClick={() => handleExpandToUnits(bundle, quantity)}
                className="flex-1 bg-sky-600 hover:bg-sky-700 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95"
              >
                Expand → {bundle.quantity * quantity} units
              </button>
              <button
                onClick={() => setBundle(null)}
                className="flex-1 py-3 rounded-xl border-2 border-sky-200 dark:border-sky-700 font-black text-[10px] uppercase tracking-widest text-sky-600 dark:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-900/20 transition-all"
              >
                Keep as {quantity} case{quantity !== 1 ? 's' : ''}
              </button>
            </div>
          </div>
        )}
          ...
          <>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="ml-auto text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-sky-500 transition-colors flex items-center gap-1.5"
              >
        ...
        {!loading && duplicateCandidates.length > 0 && (
          <div className="premium-card overflow-hidden animate-fade-in">
            <div className="p-4 bg-amber-50 dark:bg-amber-500/10 border-b border-amber-100 dark:border-amber-500/20 flex items-start gap-3">
              <svg className="w-5 h-5 text-amber-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
              </svg>
              <div>
                ...
                <p className="text-sm font-semibold text-amber-800 dark:text-amber-200">
                  These existing products look similar. Use one instead or create anyway.
                </p>
              </div>
            </div>
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {duplicateCandidates.map((c) => {
                const img = c.images?.find((i) => i.isPrimary);
                return (
                  <div key={c.id} className="flex items-center gap-4 p-4">
                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 overflow-hidden">
                      {img ? (
                        ...
                        <span className="text-sm font-black text-slate-400">{c.name.charAt(0)}</span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-black text-slate-900 dark:text-white text-sm truncate">{c.name}</p>
                      <p className="text-[10px] text-slate-400 font-medium mt-0.5">
                        {c.sku} &middot; {Math.round(c.similarity * 100)}% match
                      </p>
                    </div>
                    <button
                      onClick={() => handleUseExistingProduct(c.id)}
                      className="shrink-0 px-3 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest transition-all active:scale-95"
                    >
                      Use
                    </button>
                  </div>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 flex gap-3">
              <button
                ...
                className="flex-1 py-3 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-600 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 hover:border-red-400 hover:text-red-500 transition-all"
              >
                Create Anyway
              </button>
              <button
                onClick={() => setDuplicateCandidates([])}
                className="flex-1 py-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
              >
                Go Back
              </button>
            </div>
        ...
        {/* ── New product form ── */}
        {!loading && showAddProduct && !product && duplicateCandidates.length === 0 && (
          <ProductForm
            formData={newProduct}
            onChange={(e) =>
              setNewProduct({ ...newProduct, [e.target.name]: Number(e.target.value) || e.target.value })
            }
            onSubmit={() => handleCreateProduct(false)}
